...
import dataset_noZero #script for dataset management
...
def translate_for_categorical(vector):
    """Utility that converts an array of labels in the range [0, biggest_label]
    So that it can be passed to to_categorical.
    Args:
...
def data_and_split(directory, window, already_windowed, not_already_labelled):
    ...
    data, label, sensors = dataset_noZero.get_dataset(directory, window, already_windowed, not_already_labelled)
...
def create_model(window, classes):
    ...
    y = Conv1D(64, 7, padding="same", activation='relu')(accelerometer_input)
    y = MaxPooling1D(pool_size=2, strides=1)(y)
    y = Conv1D(64, 5, padding="valid", activation='relu')(y)
    y = MaxPooling1D(pool_size=2, strides=1)(y)
    y = Conv1D(5, 1, activation='relu')(y)
    y = Dropout(0.3)(y)
    accel_out = Flatten()(y)
...
accel_fc = 50
window = 4
classes, data, Y = data_and_split(directory, 4, False, False)
...
for train_indices, test_indices in kfold.split(Y):
    ...
    print('\nTraining...')
    history = model.fit([train_accel, train_sensor, train_times, train_proximity], trainY, epochs=20, batch_size=32, verbose=0)
    print('\nEvaluating...')
