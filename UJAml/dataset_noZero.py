...
def window_signals(list_of_tris, window_in_sec, sensor_list):
    ...
    for tris in list_of_tris:
        ...
        indices = [i for i, x in enumerate(accel_lab[:min_length]) if x == 0.0]
        ...
        accel_lab = del_list_numpy(accel_lab[:min_length], indices)

        labels_acc = np.append(labels_acc, accel_lab)
...
def align_sources(list_of_tris): #given a list of tris, truncate them at the same start and end
    for tris in list_of_tris:
        ...
        tris[2] = tris[2].truncate(before=nearest_prox_start).reset_index(drop=True)
...
def get_dataset(directory, window_size_seconds, just_read, not_labelled):
    ...
    if just_read:
        load_from = start+str(window_size_seconds)+'_seconds_noZero.npz'
        ...
        save_to = start+str(window_size_seconds)+'_seconds_noZero'
